  ...
  client.on('message', async message => {
    ...
    if(message.content.startsWith(prefix + 'config')){
      ...
      if (content.toLowerCase() === "add-invites") {
        if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send("You need `MANAGE GUILD` to add invites!");
 ...
 return message.channel.send(`**Done Added \`${amount}\` To ${user}**`);
  ...
  client.on('message', async message => {
    ...
    if(message.content.startsWith(prefix + 'config')){
      ...
      if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send("You need `MANAGE GUILD` to remove invites!");
